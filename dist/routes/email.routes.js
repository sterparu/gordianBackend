...
const checkAndRenewBalance = async (settings) => {
    const now = new Date();
...
const deductCredits = async (userId, amount) => {
    ...
    const { data: current } = await supabase_1.supabase.from('user_settings').select('remaining_credits').eq('id', userId).single();
    if (!current)
...
const filterBlacklisted = async (recipients, userId) => {
    // 1. Fetch user's blacklist
    const { data: blacklist, error } = await supabase_1.supabase
        .from('blacklist')
        .select('email')
        .eq('user_id', userId);
    if (error || !blacklist)
...
router.post('/send', async (req, res) => {
    try {
        ...
        const recipientsWithTracking = validRecipients.map((r) => {
            // Standardize input
            const email = typeof r === 'string' ? r : r.email;
        ...
        const { error: logError } = await supabase_1.supabase.from('email_logs').insert(logs);
        if (logError)
